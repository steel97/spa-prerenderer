...
public partial class SPAController : ControllerBase
{
    ...
    public ActionResult Index(string url)
    {
        ...
        var reg15 = PyiRedirectRegex();
        if (reg15.IsMatch(inp))
        {
            var matches = reg15.Matches(inp);
            if (matches.Count > 0)
            {
                var groups = matches[0].Groups;
                if (groups.Count > 1)
                {
                    _301target = $"/{groups[1]}/coin/kls";
                    skipCrawlerCheck = true;
                }
            }
        }

        var reg16 = HtnRedirectRegex();
        if (reg16.IsMatch(inp))
        {
            var matches = reg16.Matches(inp);
            if (matches.Count > 0)
            {
                var groups = matches[0].Groups;
                if (groups.Count > 1)
